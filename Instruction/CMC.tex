...
% CircleX 的参数方程
\pgfmathdeclarefunction{CircleX}{1}{%
    \pgfmathparse{8*cos(\x)}%
}

% CircleY 的参数方程
\pgfmathdeclarefunction{CircleY}{1}{%
    \pgfmathparse{8*sin(\x)}%
}

% cosh 的参数方程
\pgfmathdeclarefunction{Cosh}{1}{%
    \pgfmathparse{(e^(\x)+e^(-\x))/2}%
}
...
\pgfmathdeclarefunction{Tanh}{1}{%
    \pgfmathparse{(e^(\x)-e^(-\x))/(e^(\x)+e^(-\x))}%
}

% coth 的参数方程
\pgfmathdeclarefunction{Coth}{1}{%
    \pgfmathparse{(e^(\x)+e^(-\x))/(e^(\x)-e^(-\x))}%
}

% catenary 的参数方程
\pgfmathdeclarefunction{Catenary}{1}{%
    \pgfmathparse{15*(((e^(\x/15)+e^(-\x/15))/2)-1)}%
}

% tractrix 的参数方程
\pgfmathdeclarefunction{Tractrix}{1}{%
    \pgfmathparse{15*ln((15+sqrt(15^2-(\x)^2))/\x)-sqrt(15^2-(\x)^2)}%
}

% ferry 的参数方程
\pgfmathdeclarefunction{Ferry}{1}{%
    \pgfmathparse{0.5*\x*(((\x/20)^(-0.5))-((\x/20)^(0.5)))}%
}

% rho_1 的参数方程
\pgfmathdeclarefunction{Rho1}{1}{%
    \pgfmathparse{sqrt(2)*10*e^(pi/4-\x*pi/180)}%
}

% rho_2 的参数方程
\pgfmathdeclarefunction{Rho2}{1}{%
    \pgfmathparse{sqrt(2)*10*e^(3*pi/4-\x*pi/180)}%
}

% rho_3 的参数方程
\pgfmathdeclarefunction{Rho3}{1}{%
    \pgfmathparse{sqrt(2)*10*e^(5*pi/4-\x*pi/180)}%
}

% rho_4 的参数方程
\pgfmathdeclarefunction{Rho4}{1}{%
    \pgfmathparse{sqrt(2)*10*e^(7*pi/4-\x*pi/180)}%
}

% roadx 的参数方程
\pgfmathdeclarefunction{RoadX}{1}{%
    \pgfmathparse{8*e^(-\K*\x*pi/180)*cos(\x)}%
}

% roady 的参数方程
\pgfmathdeclarefunction{RoadY}{1}{%
    \pgfmathparse{8*e^(-\K*\x*pi/180)*sin(\x)}%
}

% roadz 的参数方程
\pgfmathdeclarefunction{RoadZ}{1}{%
    \pgfmathparse{15*(1-e^(-\K*\x*pi/180))}%
}
...
\begin{figure}[H]
\begin{center}
\begin{tikzpicture}[domain=-10:10,scale = 0.4]
\coordinate (P0) at (0,0);
\coordinate (P1) at (-3,9);
\coordinate (P2) at (0,-9);
    \draw[-latex] (-10,0) -- (10,0) node[below] {$x$};
    \draw[-latex] (0,-11) -- (0,11) node[left] {$y$};
    \draw[color=red] plot[elegant, domain=-3:3](\x,{Cosh}) 
		(P1)node[anchor = east] {$ y = \cosh(x) $};
    \draw[color=blue] plot[elegant, domain=-3:3](\x,{Sinh}) 
		node[anchor = west] {$ y = \sinh(x) $};
    \draw[color=green] plot[elegant, domain=-9:9](\x,{Tanh}) 
		node[anchor = south] {$ y = \tanh(x) $};
    \draw[color=orange] plot[elegant, domain=-9:-0.1](\x,{Coth}) 
		(P2)node[anchor = west] {$ y = \coth(x) $};
    \draw[color=orange] plot[elegant, domain=0.1:9](\x,{Coth}) ;
\draw (P0)node [below right]{$O$};
\end{tikzpicture}
\caption{双曲函数示意图}
\label{fighyp}
\end{center}
...
$\left\{\begin{array}{ll}
\dis\arccosh(x)=\ln(x\pm\sqrt{x^2-1})=\pm\ln(x+\sqrt{x^2-1}),
&
\dis\arccosh'(x)=\pm\dfrac{1}{\sqrt{x^2-1}}\\
\dis\arcsinh(x)=\ln(x+\sqrt{x^2+1}),
&
\dis\arccosh'(x)=\dfrac{1}{\sqrt{x^2+1}}\\
\dis\arctanh(x)=\dfrac{1}{2}\ln\dfrac{1+x}{1-x},
&
\arctanh'(x)=\dfrac{1}{1-x^2}\vspace{0.15cm}\\
\dis\arccoth(x)=\dfrac{1}{2}\ln\dfrac{x+1}{x-1},
&
\arccoth'(x)=\dfrac{1}{1-x^2}\\
\end{array}\right.$

\vspace{0.3cm}

四种反双曲函数的图形如图\ref{figarchyp}所示。

\begin{figure}[!htbp]
\begin{center}
\begin{tikzpicture}[domain=-10:10,scale = 0.3]
\coordinate (P0) at (0,0);
\coordinate (P1) at (10,-4);
\coordinate (P2) at (-1,-8);
\coordinate (P3) at (1,8);
    \draw[-latex] (-11,0) -- (11,0) node[below] {$x$};
    \draw[-latex] (0,-10) -- (0,10) node[left] {$y$};
    \draw[color=red] plot[elegant, domain=-3:3]({Cosh},\x) 
		(P1)node[anchor = east] {$ y = \arccosh(x) $};
    \draw[color=blue] plot[elegant, domain=-3:3]({Sinh},\x) 
		node[anchor = south] {$ y = \arcsinh(x) $};
    \draw[color=green] plot[elegant, domain=-9:9]({Tanh},\x) 
		(P3)node[anchor = west] {$ y = \arctanh(x) $};
    \draw[color=orange] plot[elegant, domain=-9:-0.1]({Coth},\x) 
		(P2)node[anchor = east] {$ y = \arccoth(x) $};
    \draw[color=orange] plot[elegant, domain=0.1:9]({Coth},\x) ;
\draw (P0)node [below right]{$O$};
\end{tikzpicture}
\caption{反双曲函数示意图}
\label{figarchyp}
...
\textbf{例}：若$z=z(x,y),$质点沿梯度方向行走，则$(\dif x,\dif y)=\grad f(x,y)$

\no4.几种特殊图形

(1)悬链线：$y=a\left(\cosh\dfrac{x}{a}-1\right),\ a>0.$如图\ref{figcat}所示。
...
\coordinate (P0) at (0,0);
    \draw[-latex] (-11,0) -- (11,0) node[below] {$x$};
    \draw[-latex] (0,-10) -- (0,10) node[left] {$y$};
    \draw[color=red] plot[elegant, domain=-9.8:9.8](\x,{Catenary}) 
		node[anchor = south] {$ y =a\left(\cosh\dfrac{x}{a}-1\right) $};
\draw (P0)node [below left]{$O$};
\end{tikzpicture}
\caption{悬链线}
\label{figcat}
...
(2)曳物线：$\dis x=a\ln\dfrac{a+\sqrt{a^2-y^2}}{y}-\dis\sqrt{a^2-y^2},\ a$ is the length of the line. 如图\ref{figtra}所示。

\begin{figure}[!htbp]
\begin{center}
\begin{tikzpicture}[domain=-10:10,scale = 0.3]
\coordinate (P0) at (0,0);
    \draw[-latex] (-2,0) -- (39,0) node[below] {$x$};
    \draw[-latex] (0,-2) -- (0,19) node[left] {$y$};
    \draw[color=blue] plot[elegant, domain=1:14]({Tractrix},\x) 
		node[anchor = west] {$ x=a\ln\dfrac{a+\sqrt{a^2-y^2}}{y}-\dis\sqrt{a^2-y^2} $};
\draw (P0)node [below left]{$O$};
\end{tikzpicture}
\caption{曳物线}
\label{figtra}
...
(3)摆渡路线：$y=\dfrac{x}{2}\left[\left(\dfrac{x}{h}\right)^{-\tfrac{a}{b}}-
\left(\dfrac{x}{h}\right)^{\tfrac{a}{b}}\right],\ a$ is the velocity of water, $b$ is the velocity
of the ship and $h$ is the width of the river. 如图\ref{figfer}所示。
...
\coordinate (P1) at (20,3);
    \draw[-latex] (-2,0) -- (25,0) node[below] {$x$};
    \draw[-latex] (0,-2) -- (0,10) node[left] {$y$};
    \draw[color=blue] plot[elegant, domain=0.01:20](\x,{Ferry}) 
		(P1)node[anchor = south] {$ y=\dfrac{x}{2}\left[\left(\dfrac{x}{h}\right)^{-\tfrac{a}{b}}-\left(\dfrac{x}{h}\right)^{\tfrac{a}{b}}\right] $};
\draw (P0)node [below left]{$O$};
\end{tikzpicture}
\caption{摆渡路线}
\label{figfer}
...
$\rho_1=\sqrt{2}ae^{\tfrac{\pi}{4}-\theta},\ \theta\gs\dfrac{\pi}{4},
\quad
\rho_2=\sqrt{2}ae^{\tfrac{3\pi}{4}-\theta},\ \theta\gs\dfrac{3\pi}{4},$

\vspace{0.2cm}

$\rho_3=\sqrt{2}ae^{\tfrac{5\pi}{4}-\theta},\ \theta\gs\dfrac{5\pi}{4},
\quad
\rho_4=\sqrt{2}ae^{\tfrac{7\pi}{4}-\theta},\ \theta\gs\dfrac{7\pi}{4}.$

如图\ref{figpol}所示。

\begin{figure}[!htbp]
\begin{center}
\begin{tikzpicture}[domain=-10:10,scale = 0.25]
\coordinate (P0) at (0,0);
\coordinate (P1) at (10,10);
\coordinate (P2) at (-10,10);
\coordinate (P3) at (-10,-10);
\coordinate (P4) at (10,-10);
    \draw[-latex] (-11,0) -- (11,0) node[below] {$x$};
    \draw[-latex] (0,-11) -- (0,11) node[left] {$y$};
    \draw[color=red] plot[elegant, domain=180/4:20*180](\x:{Rho1}) 
		(P1)node[anchor = south] {$ \rho_1=\sqrt{2}ae^{\tfrac{\pi}{4}-\theta}$};
    \draw[color=red] plot[elegant, domain=180*3/4:20*180](\x:{Rho2}) 
		(P2)node[anchor = east] {$ \rho_2=\sqrt{2}ae^{\tfrac{3\pi}{4}-\theta}$};
    \draw[color=red] plot[elegant, domain=180*5/4:20*180](\x:{Rho3}) 
		(P3)node[anchor = north] {$ \rho_3=\sqrt{2}ae^{\tfrac{5\pi}{4}-\theta}$};
    \draw[color=red] plot[elegant, domain=180*7/4:20*180](\x:{Rho4}) 
		(P4)node[anchor = west] {$ \rho_4=\sqrt{2}ae^{\tfrac{7\pi}{4}-\theta}$};
\draw (P0)node [below left]{$O$};
\end{tikzpicture}
\caption{四人追逐线路}
\label{figpol}
...
(5)盘山公路：$\dis z=h-\dfrac{h}{R}\sqrt{x^2+y^2},\ 0\ls z\ls h,$
公路倾角$\alpha,\ \tan \alpha<\dfrac{h}{R},\ 
\dis\dfrac{1}{k}=\sqrt{\left(\dfrac{h}{R}\right)^2\cot^2\alpha-1},$

\vspace{0.2cm}

$L:\left\{\begin{array}{l}
x=Re^{-k\theta}\cos \theta,\\
 y=Re^{-k\theta}\sin \theta,\\
z=h\left(1-e^{-k\theta}\right),\\
\end{array}\right. \theta\gs0.$
如图\ref{figroa}所示。
...
\coordinate (P1) at (1,15,0);
     \draw[-latex] (-12,0,0) -- (12,0,0) node[below] {$y$};
     \draw[-latex] (0,-5,0) -- (0,18,0) node[left] {$z$};
     \draw[-latex] (0,0,-15) -- (0,0,15) node[left] {$x$};
     \draw[color=red] plot[elegant, domain=0:7*360]({RoadY},{RoadZ},{RoadX}) 
		(P1)node[anchor = west] {$L:\left\{\begin{array}{l}
x=Re^{-k\theta}\cos \theta,\\
 y=Re^{-k\theta}\sin \theta,\\
z=h\left(1-e^{-k\theta}\right),\\
\end{array}\right. \theta\gs0.$};
	\draw[color=blue] plot[elegant, domain=0:360]({CircleX},0,{CircleY}) ;
     \draw[color=blue] (-8,0,0) -- (0,15,0) ;
     \draw[color=blue] (8,0,0) -- (0,15,0) ;
     \draw[color=blue] (0,0,8) -- (0,15,0) ;
     \draw[color=blue] (0,0,-8) -- (0,15,0) ;
     \draw[color=blue] (4*1.414,0,-4*1.414) -- (0,15,0) ;
     \draw[color=blue] (4*1.414,0,4*1.414) -- (0,15,0) ;
     \draw[color=blue] (-4*1.414,0,4*1.414) -- (0,15,0) ;
     \draw[color=blue] (-4*1.414,0,-4*1.414) -- (0,15,0) ;
\draw (P0)node [below right]{$O$};
